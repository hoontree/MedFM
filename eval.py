...
from utils.visualize import denormalize_image, plot_segmentation_panel, _extract_filenames
...
def visualize_single_sample(
    ...
    plot_segmentation_panel(
        image, gt, pred, save_path,
        num_classes=1, metrics=metrics, overlay=False,
        filename=filename,
    )
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(
            ...
            for i in range(images.size(0)):
                ...
                file_label = fname or f"{sample_idx:04d}"
                vis_path = save_dir / f"sample_{file_label}.png"
