...
def visualize_predictions(
    ...
    with torch.no_grad():
        for batch in tqdm(dataloader, desc=f"Visualizing {phase_name} predictions"):
            ...
            masks = batch[1].to(device)  # [B, C, H, W]

            # For multi-class visualization, convert masks back to indices [B, 1, H, W]
            if num_classes > 1:
            ...
            for i in range(images_np.shape[0]):
                ...
                colored_pred = cmap(
                    pred / (num_classes - 1) if num_classes > 1 else pred
                )
...
def visualize_from_predictions(
    ...
    for images, preds, masks in zip(images_list, preds_list, masks_list):
        # masks is [B, C, H, W]. Convert to [B, 1, H, W] indices for visualization if multi-class
