...
class DiceLoss(nn.Module):
    ...
    def forward(self, inputs, target, weight=None, softmax=False):
        ...
        inputs: [B, C, H, W]
        target:
          - multi-class: [B, H, W] (int) -> one-hot
          - binary(n_classes == 1): [B, 1, H, W] (float 0/1)
        """
        if self.n_classes == 2:
            # ---- Binary Dice ----
            # inputs: logits [B,1,H,W] => sigmoid로 확률화
            probs = torch.sigmoid(inputs)
            # target: 이미 [B,1,H,W] float 0/1 이라고 가정
            assert probs.size() == target.size(), \
                f'predict {probs.size()} & target {target.size()} shape do not match'
            return self._dice_loss(probs, target)
