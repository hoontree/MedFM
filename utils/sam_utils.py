...
class Focal_loss(nn.Module):
    ...
    def forward(self, preds, labels):
        ...
        )  # torch.low(1 - preds_softmax) == (1 - pt) ** r
...
class DiceLoss(nn.Module):
    ...
    def forward(self, inputs, target, weight=None, softmax=False, sigmoid=True):
        """
    ...
    if len(image.shape) == 3:
        ...
        for ind in range(image.shape[0]):
            ...
            if x != input_size[0] or y != input_size[1]:
                slice = zoom(
                    slice, (input_size[0] / x, input_size[1] / y), order=3
                )  # previous using 0
            new_x, new_y = (
                slice.shape[0],
                slice.shape[1],
            ...
            if new_x != patch_size[0] or new_y != patch_size[1]:
                slice = zoom(
                    slice, (patch_size[0] / new_x, patch_size[1] / new_y), order=3
