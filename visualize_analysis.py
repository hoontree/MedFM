...
def load_student_model(cfg, device):
    ...
    if cfg.get("student_checkpoint"):
        ...
        if "adapter_state_dict" in checkpoint:
            ...
            print(
                "Warning: Feature Adapter weights not found in checkpoint. Using random initialization."
            )
...
def visualize_attention(image, student_attn, teacher_attn, save_path):
    ...
    s_map_resized = (
        F.interpolate(
            torch.tensor(s_map).unsqueeze(0).unsqueeze(0),
            size=(224, 224),
            mode="bilinear",
        )
        .squeeze()
    ...
    t_map_resized = (
        F.interpolate(
            torch.tensor(t_map).unsqueeze(0).unsqueeze(0),
            size=(224, 224),
            mode="bilinear",
        )
        .squeeze()
...
def main(cfg):
    ...
    with torch.no_grad():
        for i, (images, masks, _) in enumerate(tqdm(val_loader)):
            ...
            # Forward Teacher
            # SAM image encoder
            t_feat = teacher.sam.image_encoder(
                images
            )  # [B, 256, 64, 64] -> Wait, input 224?
            # If input 224, patch size 16 -> 14x14.
            # Let's verify shape.

            # Get Attention
            t_attn_flat = teacher.sam.image_encoder.blocks[
                -1
            ].attn.last_attn  # [B*Heads, N, N]
            num_heads = teacher.sam.image_encoder.blocks[-1].attn.num_heads
            t_attn = t_attn_flat.view(
                images.shape[0], num_heads, t_attn_flat.shape[-1], t_attn_flat.shape[-1]
            )
