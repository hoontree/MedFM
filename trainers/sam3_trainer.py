...
from model.sam3.model.data_misc import BatchedDatapoint
from model.sam3.model.model_misc import SAM3Output
from model.sam3.model.utils.misc import copy_data_to_device

from model.sam3.train.optim.optimizer import construct_optimizer

from model.sam3.train.utils.checkpoint_utils import (
...
from model.sam3.train.utils.distributed import all_reduce_max, barrier, get_rank

from model.sam3.train.utils.logger import Logger, setup_logging
from model.sam3.train.utils.train_utils import (
    AverageMeter,
    collect_dict_keys,
    DurationMeter,
    get_amp_type,
...
class Trainer:
    ...
    def val_epoch(self, val_loader, phase):
        ...
        # Visualize masks on rank 0
        if self.distributed_rank == 0:
            self._visualize_and_log_masks(val_loader, curr_models[0])

        self._reset_meters(curr_phases)
        logging.info(f"Meters: {out_dict}")
        return out_dict

    def _visualize_and_log_masks(self, loader, model):
        """Visualize and log masks for the first batch of the loader."""
        model.eval()
        try:
        ...
        key, batch_data = batch.popitem()
        batch_data = copy_data_to_device(batch_data, self.device, non_blocking=True)
        ...
        with torch.no_grad():
            with torch.amp.autocast(
                ...
                find_stages = model(batch_data)
                find_targets = [
                    unwrap_ddp_if_wrapped(model).back_convert(x)
                    for x in batch_data.find_targets
                ]

        # Get visualization
        from model.sam3.visualization_utils import render_masklet_frame

        img_batch = batch_data.img_batch
        # Normalization parameters for ultrasound_seg (usually [0.5, 0.5, 0.5])
        mean = np.array([0.5, 0.5, 0.5])
        std = np.array([0.5, 0.5, 0.5])

        num_vis = min(len(img_batch), 4)  # Visualize up to 4 images

        # Prepare GT data from the first stage
        if len(find_targets) > 0:
            targets_stage0 = find_targets[0]
            gt_masks_all = targets_stage0["masks"]
            gt_boxes_all = targets_stage0["boxes"]
            num_boxes_per_img = targets_stage0["num_boxes"]
        else:
            # Fallback if no targets (should not happen in val with targets)
            gt_masks_all = None
            gt_boxes_all = None
            num_boxes_per_img = torch.zeros(len(img_batch), dtype=torch.long)

        gt_offset = 0
        for i in range(num_vis):
            img = img_batch[i].cpu().numpy().transpose(1, 2, 0)
            img = (img * std + mean).clip(0, 1)

            # Ground Truth
            n_gt = int(num_boxes_per_img[i])
            if gt_masks_all is not None and n_gt > 0:
                gt_masks = gt_masks_all[gt_offset : gt_offset + n_gt]
                gt_boxes = gt_boxes_all[gt_offset : gt_offset + n_gt]
            else:
                gt_masks = torch.zeros(
                    (0, img.shape[0], img.shape[1]), dtype=torch.bool
                )
            ...
            gt_outputs = {
                "out_boxes_xywh": gt_boxes.cpu().numpy(),
                "out_probs": [1.0] * len(gt_boxes),
                "out_obj_ids": list(range(len(gt_boxes))),
                "out_binary_masks": gt_masks.cpu().numpy(),
            }
            gt_vis = render_masklet_frame(img, gt_outputs, alpha=0.5)

            # Prediction
            # Note: find_stages is usually a list of SAM3Output (one per stage)
            # We take the last stage for visualization
            last_stage = find_stages[-1]
            pred_masks = last_stage["pred_masks"][i]
            pred_boxes = last_stage["pred_boxes"][i]
            pred_probs = last_stage["pred_logits"][i].sigmoid().squeeze(-1)

            # Filter by a small threshold for visualization
            mask_threshold = 0.5
            keep = pred_probs > 0.3
            ...
            pred_outputs = {
                "out_boxes_xywh": pred_boxes.float().cpu().numpy(),
                "out_probs": pred_probs.float().cpu().numpy(),
                "out_obj_ids": list(range(len(pred_boxes))),
                "out_binary_masks": (pred_masks.sigmoid() > mask_threshold)
                .cpu()
                .numpy(),
            }
            pred_vis = render_masklet_frame(img, pred_outputs, alpha=0.5)

            # Combine GT and Pred side-by-side
            combined_vis = np.concatenate([gt_vis, pred_vis], axis=1)

            # Save locally
            vis_dir = os.path.join(self.logging_conf.log_dir, "visualizations")
            makedir(vis_dir)
            vis_path = os.path.join(vis_dir, f"epoch_{self.epoch}_sample_{i}.png")
            from PIL import Image

            Image.fromarray(combined_vis).save(vis_path)

            # Log to WandB
            self.logger.log_image(
                f"Visualizations/Sample_{i}",
                combined_vis,
                step=self.epoch,
                caption=f"Epoch {self.epoch}, Sample {i} (Left: GT, Right: Pred)",
            )
    ...
    def _log_meters_and_save_best_ckpts(self, phases: List[str]):
        ...
        for key, meter in self._get_meters(phases).items():
            ...
            for meter_subkey, meter_value in meter_output.items():
                out_dict[os.path.join("Meters", key, meter_subkey)] = meter_value
